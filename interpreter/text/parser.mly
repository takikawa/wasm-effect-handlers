...
%token NUM_TYPE FUNCREF EXTERNREF EXTERN MUT EXNREF
%token UNREACHABLE NOP DROP SELECT
%token BLOCK END IF THEN ELSE LOOP BR BR_IF BR_TABLE BR_ON_EXN
...
exception_fields :
  | exception_type
    { fun c x at ->
      [{xtype = ExceptionType ($1 c)} @@ at], [], [] }
  | inline_import exception_type  /* Sugar */
    { fun c x at ->
      [], [{ module_name = fst $1; item_name = snd $1;
             idesc = ExceptionImport ($2 c) @@ at } @@ at], [] }
